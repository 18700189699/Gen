...
import com.uxsino.Netty.ServerNetty.GetFile;
import com.uxsino.Netty.ServerNetty.MyClient;
import com.uxsino.Netty.ServerNetty.MyClientHandlerAdapter;
...
public class TestController {
	...
	public Map<String,Object> getByStatus() throws InterruptedException {
		Map<String,Object> map = new HashMap();
//		final MyClient c = MyClient.getInstance();
		MyClientHandlerAdapter myClientHandlerAdapter = new MyClientHandlerAdapter();
		MyClient c = MyClient.getInstance(myClientHandlerAdapter);
		ChannelFuture cf = c.getChannelFuture();
		String request ="hello Service  我是客户端$_";
		cf.channel().writeAndFlush(Unpooled.copiedBuffer(request.getBytes()));
		cf.channel().closeFuture().sync();
		System.out.println(cf.channel().isActive());//是否是活着的
		System.out.println(cf.channel().isOpen());//是否是打开的
		Object a = myClientHandlerAdapter.getResponse();
		map.put("a",a);
		return map;
	}
